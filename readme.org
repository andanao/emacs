...
=ghostel= keys a terminal by the buffer name it was started under, so a wrapper that binds
=ghostel-buffer-name= gets a slot of its own and =SPC s n= can still open ordinary shells beside it.

#+begin_src emacs-lisp
(defvar ghostel-buffer-name)

(defconst ads/ghostel-popup-name "*ghostel-popup*"
  "Buffer name of the always-on terminal, and its identity to `ghostel'.")
...
elsewhere, and starts it the first time."
  (interactive)
  (let ((window (get-buffer-window ads/ghostel-popup-name)))
    (cond
...
;; that, which is the whole reason this lands in a side window.
(add-to-list 'display-buffer-alist
             `(,(regexp-quote ads/ghostel-popup-name)
