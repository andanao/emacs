...
(defun ads/agent-shell-knockknock--on-turn-complete (event)
  ...
  (unless (agent-shell-knockknock--shell-visible-p (current-buffer))
    (knockknock-notify
     :title (ads/agent-shell--session-name (current-buffer))
...
A posframe in the top right of the frame, which is the one way anything in here interrupts me:
[[*notify when a session finishes][agent shells]] today, meetings and the [[file:~/git/konfig/readme.org::*Claude Bot inbox loop][inbox loop]] later.  =SPC x= puts it away.
...
(defun ads/notify--poshandler (info)
  "Top right of the frame, inset far enough to look deliberate."
  (let ((inset 16))
...
(defun ads/notify (title message icon)
  "Put MESSAGE on screen under TITLE, with ICON, until I dismiss it."
  (knockknock-notify :title title :message message :icon icon))

(ads/leader-keys "x" '(knockknock-close :wk "dismiss notification"))
#+end_src
