...
=latex= enables =#+begin_src latex :file diagram.svg= blocks, which render TikZ/pgfplots out to
standalone image files on disk -- the thing you want when the diagram needs to leave Emacs.  See
[[*org latex preview][org latex preview]] for the inline case.

Note that =d2= is deliberately absent: [[*d2-mode][d2-mode]] provides =org-babel-execute:d2= itself and has no
=ob-d2= feature to require, so listing it here would error.
