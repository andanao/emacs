...
breif testing this feels much smoother and faster than vterm.
...
(use-package toggl
  ...
  (toggl-mode 1)
  (toggl-org-mode 1)
  ;; The transient is the hub: start, stop, edits and the entry list all hang
  ;; off it.  Stop keeps its own key because it is the one thing worth hitting
  ;; without reading a menu first.
  (ads/leader-keys
    "C-t" 'toggl-dispatch
    "C-S-t" 'toggl-stop)
  ;; Most starts here come from the curated list rather than a free prompt, so
  ;; put that in the transient next to the generic one.
  (transient-append-suffix 'toggl-dispatch '(0 0 -1)
    '("f" "Favourite timer" ads/toggl-quick-start)))
#+end_src

**** quick timers

Most tracking now comes from clocking in, but some things I start by hand and
never want to retype.  These used to be CLI argument strings passed to
~toggl-start-timer~; they are now description/project pairs handed to the real
command, so a typo'd project name fails loudly instead of starting an
unprojected entry.

Work-specific entries get appended to this in [[file:~/git/konfig/readme.org][konfig]].

#+begin_src emacs-lisp
(defvar ads/toggl-quick-timers
  '(;; Personal productivity
    ...
    ("Chess"        "Chess")
    ("Editing"      "Photography"))
  "Timers started by hand, as (DESCRIPTION PROJECT &optional BILLABLE).")

(defun ads/toggl-quick-start ()
  "Start a timer from `ads/toggl-quick-timers'."
  (interactive)
  (let* ((choice (completing-read "Toggl: " (mapcar #'car ads/toggl-quick-timers) nil t))
         (timer (assoc choice ads/toggl-quick-timers)))
    (toggl-start (nth 0 timer) (nth 1 timer) (nth 2 timer))))
#+end_src
