...
Buttons hang their binding off a `keymap' text property rather than
`button-1', so there is nothing for `push-button' to find - look the
RET up ourselves.  Falls through to the next item when point is on
nothing expandable, so TAB still walks the buffer."
  (interactive)
