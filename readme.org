...
(use-package default-text-scale
  ...
  (transient-define-prefix ads/text-scale-transient ()
    ...
    ["Text Scale"
     ("k" "increase" default-text-scale-increase)
     ("j" "decrease" default-text-scale-decrease)
     ("h" "reset" default-text-scale-reset :transient nil)
     ("q" "quit" transient-quit-one :transient nil)
     ("<escape>" "quit" transient-quit-one :transient nil)])
...
(use-package vterm
    :config
    ;; Use emacs state so C-c works as terminal break
    ;; and zsh's vim bindings (set -o vi) handle editing
    (evil-set-initial-state 'vterm-mode 'emacs)

    ;; Copy mode still uses vim bindings via evil-collection
