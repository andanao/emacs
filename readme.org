...
Recent vertico builds call ~set-local~, which only exists in Emacs 31+.  Until I
upgrade this shim keeps it working on Emacs 30.  Remove it once on Emacs 31+.

#+begin_src emacs-lisp
;; TODO: remove this shim after upgrading to Emacs 31+
(if (>= emacs-major-version 31)
    (display-warning 'init "Emacs 31+ detected: remove the `set-local' shim in the vertico config" :warning)
  (unless (fboundp 'set-local)
    (defun set-local (variable value)
      "Compat shim for Emacs 31's `set-local': set buffer-local VARIABLE to VALUE."
