...
#+end_src
** org-timegrid
[[https://github.com/Gleek/org-timegrid][org-timegrid]] draws the day's timestamped org entries onto a grid of hours instead of listing
them, which is the calendar view the agenda never gives me.

Here on trial, so this is the install and nothing else — no leader key and nothing else in the
config naming it, which makes deleting this block the whole uninstall.  Its commands are
autoloaded, so =M-x= on the =org-timegrid-= prefix is how to reach them until I know whether it
stays.
