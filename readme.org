...
The colour runs through the border, the title and the icon, which is the look knockknock shows
off in its own screenshots.  The agent's logo survives it because ~:icon-file~ embeds the image
where a nerd-font glyph would be filled with the colour, so the tint only ever reaches glyphs —
the earlier green blob came from tinting the PNG myself.  Green is the whole of it while agent
shells are the only source; blue for meetings and plain for the agenda arrive with the things
that send them.
  ...
  (let* ((name (format "*knockknock %s %s*"
    ...
    (with-selected-frame frame
      (let ((knockknock--buffer name))
        (apply #'knockknock--notify-internal (ads/notify-entry-args entry))))
...
`ads/notify-visit' calls to go to whatever sent this."
  ...
  (setq ads/notify--stack
        (append ads/notify--stack
                (list (ads/notify--entry
                       ...
                       :args (ads/notify--without args :action)
