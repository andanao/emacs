...
Start it with =M-x org-noter= from either the PDF or an org heading carrying a =:NOTER_DOCUMENT:=
property; =i= takes a note on the current page, =M-i= on the current selection.

#+begin_src emacs-lisp
(use-package org-noter
  :after (pdf-tools org)
  :custom
  (org-noter-notes-search-path (list (concat org-directory "noter/")))
