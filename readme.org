...
(use-package general
 ...
 (ads/leader-keys
   ...
   "er" '((lambda () (interactive) (find-file (concat ads/user-dir-readme)))
           :wk "Readme Config")


   "e." '((lambda () (interactive) (find-file ads/user-dir-config))
           :wk ".emacs Config")
...
** Toggl time tracking
:PROPERTIES:
:ID:       2494c76c-687e-4b3d-9e59-f469712a44a1
:END:

I don't want to build something myself, I'm using the rust toggl cli to do this, I've made a list of branches under ~toggl-cli-directory~ and maintain a config file that ends up symlinked with anytihng under the ~AppData/labs/toggl-cli/config/~ dir to the main config file.

#+begin_src emacs-lisp

(setq ads/toggl-cli-directory (concat ads/git-dir "/toggl-cli"))

(defun ads/toggl-get-saved-timers ()
  (setq ads/toggl-saved-timers
    (delete "*"
      (delete "main"
        (string-split
          (shell-command-to-string
	   (concat
            "cd " ads/toggl-cli-directory " && git branch --list")))))))

(ads/toggl-get-saved-timers)

(defun ads/toggl-start-interactive ()
  (interactive)
  (let
    ((timer-type (completing-read "" ads/toggl-saved-timers)))
    (async-shell-command
      (concat
         "cd " ads/toggl-cli-directory
         " && git checkout " timer-type
         " && toggl start"))
    (message (concat "Toggl started: " timer-type))))

(defun ads/toggl--command (command)
  (async-shell-command
    (concat
      "cd " ads/toggl-cli-directory
      " && toggl " command))
...
(defun ads/toggl-stop ()
  (interactive)
  (ads/toggl--command "stop")
  )

(defun ads/toggl-resume ()
  (interactive)
  (ads/toggl--command "continue")
  )


(defun ads/toggl-config ()
    (interactive)
    (find-file
     "c:/Users/adanaos/AppData/Local/labs/toggl-cli/config/QzpcdXNlcnNcYWRhbmFvc1xnaXQ=.toml"))

(ads/leader-keys
    "j" '(:ignore t :wk "Toggl")
    "et" '(ads/toggl-config :wk "config")
    "jt" '(ads/toggl-config :wk "config")
    "jj" '(ads/toggl-start-interactive :wk "start")
    "js" '(ads/toggl-stop :wk "stop")
    "jr" '(ads/toggl-resume :wk "resume")
)
...
** Bookmarks
:PROPERTIES:
:ID:       782698d3-b578-4986-acf8-5050af7695a2
:END:

#+begin_src emacs-lisp
(evil-global-set-key 'normal (kbd "C-x C-b") 'bookmark-jump)
#+end_src
** Helpful
:PROPERTIES:
...
;;   (dired-async-mode 1))
#+end_src
** Tangle emacs refactor on save
:PROPERTIES:
:ID:       3beef06b-4713-4951-ac8e-291a0ef76617
:END:

#+begin_src emacs-lisp
(add-hook 'after-save-hook
	  (lambda ()
	    (let ((refactor-file "c:/Users/adanaos/git/emacs-refactor/emacs.org"))
	    (when (string= buffer-file-truename refactor-file)
	      (org-babel-tangle refactor-file))
	    )))
#+end_src
...
** Docker
:PROPERTIES:
:ID:       e32ea12e-675a-45cb-b7b5-3f23972ce755
:END:
#+begin_src emacs-lisp
...
** Protobuf
:PROPERTIES:
:ID:       2dbb9123-3699-42e6-aa58-aaabe04b05fe
:END:
...
** Kmonad
:PROPERTIES:
:ID:       1a6cdb25-9991-4265-8411-0d377c9801b3
:END:
#+begin_src emacs-lisp
;; (package-vc-install
