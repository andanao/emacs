...
not the best way to do it but I don't want to build a new system, I've made a list of branches under ~toggl-cli-directory~
...
(defun ads/toggl-config ()
    (interactive)
    (find-file "c:/Users/adanaos/AppData/Local/labs/toggl-cli/config/QzpcdXNlcnNcYWRhbmFvc1xnaXQ=.toml"))
(ads/leader-keys "et" '(ads/toggl-config :wk "Open toggl config"))
