...
Three minutes before anything tagged =:meeting:=, through the same [[*knockknock][posframe]] the agent shells
use.  =SPC X= opens the heading it came from.

~appt~ does the work — a filter function on ~org-agenda-to-appt~ picks the tag out of the entry's
~tags~ property, and an ~APPT_WARNTIME~ property overrides the lead per meeting for free.  Its
beep and its ten-second teardown are both off, because [[*knockknock][nothing of mine auto-dismisses]].
...
no date on it and only prunes what is already past, so without it today's meetings read as
tomorrow's and fire again at the same clock time while tomorrow's are never added at all.
Rebuilt on every agenda build too, so rescheduling something takes effect without waiting.

The icon is the glyph the [[*roam-categories][category]] already starts with, so a notification looks like its line in
the agenda.  The blue is the one thing that marks it as not an agent.
...
(defvar ads/notify-appt--entries (make-hash-table :test 'equal)
  "Appt message -> plist of :file, :point and :category behind it.")
...
(defun ads/notify-appt--meeting-p (x)
  "Non-nil when agenda entry X is a meeting, remembering where it lives."
  (when (member "meeting" (get-text-property 1 'tags x))
...
(defun ads/notify-appt-refresh (&rest _)
  "Rebuild today's appt list from the `:meeting:'-tagged agenda entries."
  (interactive)
  (clrhash ads/notify-appt--entries)
  (org-agenda-to-appt t #'ads/notify-appt--meeting-p))
   ...
   (lambda (m s)
     (let ((place (gethash s ads/notify-appt--entries)))
       (ads/notify--broadcast
        ...
        :border (ads/modus-color 'blue)
        :background (ads/modus-color 'bg-blue-nuanced)
...
;; Appended, so it lands after `ads/org-agenda-files-update' has claimed
;; `org-agenda-files' - there is nothing to scan before that runs.
(add-hook 'after-init-hook #'ads/notify-appt-refresh t)
#+end_src
