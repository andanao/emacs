...
a tangled =~/.config/rustfmt/rustfmt.toml=. The explicit =--config-path= overrides any
...
(use-package apheleia
  ...
  (setf (alist-get 'rustfmt apheleia-formatters)
        `(,(expand-file-name
            "~/.rustup/toolchains/nightly-aarch64-apple-darwin/bin/rustfmt")
          "--quiet" "--emit" "stdout"
          "--config-path" ,(expand-file-name "~/.config/rustfmt/rustfmt.toml"))))
#+end_src
...
** lsp-rust
Rust-analyzer configuration for lsp-mode. Project-specific overrides can be
set via .dir-locals.el (see kode-dir-locals.el for Bazel projects).

#+begin_src emacs-lisp
(with-eval-after-load 'lsp-rust
  ;; Use stable toolchain rust-analyzer by default
  (setq lsp-rust-analyzer-server-command
        (list (expand-file-name "~/.rustup/toolchains/stable-aarch64-apple-darwin/bin/rust-analyzer"))))
#+end_src
...
** Homebrew PATH
GUI apps on macOS don't inherit the shell's PATH. Add Homebrew's bin directory to both =exec-path= (for Emacs finding executables) and =PATH= environment variable (for subprocesses like git hooks).
#+begin_src emacs-lisp
