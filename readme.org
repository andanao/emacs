...
#+STARTUP: overview
#+TAGS: external(e) core(c) custom(m)
...
The cookie has to be the literal first line of the tangled file, so this block
sets ~:comments no~ — otherwise org prepends its link comment and Emacs ignores
the cookie sitting on line 2.
...
First block of init.el, so it carries the lexical-binding cookie.  ~:comments no~
keeps org's link comment from displacing it off line 1.
...
#+end_src
** replace-match case conversion fix

This is what was turning archived org entries into ALLCAPS, and it is an Emacs bug rather than an
org one.  ~replace-match~ with ~FIXEDCASE~ nil reads the case of the text it matched and, if that text
was all upper case, upcases the region it just wrote.  It works out that region *after* doing the
edit, from the global search registers - and the edit runs ~after-change-functions~, whose handlers
run regexp searches of their own and clobber those registers.  So the upcase lands on whatever the
last handler happened to match, which is a random span of the buffer nowhere near the replacement.

~org-todo~ walks straight into it.  It swaps the keyword with a bare ~(replace-match "")~ (org.el
~9816~, inside ~org-fold-core-ignore-modifications~, which deliberately leaves the change hooks on),
and the text it matches is a TODO keyword - always all caps - so the ~all_caps~ branch is taken every
single time.  Registers before and after the call, on a heading at position 21:

: BEFORE: md=(21 27 22 26 26 27)        ; " TODO ", correct
: AFTER : md=(116 125 116 116 116 118)  ; clobbered by an after-change handler

and 93 characters starting at 22 get upcased.  Reproduces with no org in sight:

#+begin_example
(with-temp-buffer
  (insert "* TODO Test and some lower case words")
  (goto-char 2)
  (add-hook 'after-change-functions
            (lambda (&rest _) (save-excursion (goto-char (point-min))
                                              (re-search-forward "lower case" nil t)))
            nil t)
  (looking-at " TODO ")
  (replace-match "")            ; => "* TEST AND SOME LOWER CASE WORDS"
  (buffer-string))
#+end_example

~C-c C-t~, ~S-<right>~ and ~SPC o X~ all go through ~org-todo~, which is why every one of them did it,
why the ~CLOSED~ stamp written a moment later is correctly cased, and why it only bit entries big
enough for the clobbered registers to land inside them.

An empty replacement has nothing to case-convert, so forcing ~FIXEDCASE~ there is a no-op except
that it skips the broken branch entirely.  Filter on the arguments rather than advising ~org-todo~,
because the same pattern is reachable from anything that empties a match.
...
Anything that is only a recolour belongs here rather than in [[*modus-tweaks][modus-tweaks]], because the theme re-resolves
palette overrides itself on every load and they cannot go stale when you toggle.
...
Sizes and inherits only; the few colours still needed come from ~ads/modus-color~ because
~modus-themes-with-colors~ needs =cl-seq= at runtime and silently swallows the =void-function=
error when it is missing, which is what used to leave this whole block unapplied.
...
~enable-theme-functions~ runs inside ~load-theme~ itself, so the tweaks land on the startup theme
and on every toggle, and no other theme hook erroring first can skip them.
...
A toggle sometimes lands half applied — the agenda came back white on white — and the way out
is to load the theme again from the top, palette overrides, tweaks and every
[[*rebuild on theme change][after-load hook]] included.  Every forced
reload appends an entry to ~ads/modus-reload-log~ with the tail of =*Messages*=, because
needing it at all is a bug and that log is the only evidence left of which hook died.
...
[[*doom-modeline][doom-modeline]] already prints the project name; the tint is the same thing read without
reading.  The remap is buffer-local and the modeline is drawn per window, so two projects
side by side show two colours at once.  Not the header line — =lsp-headerline-breadcrumb=
already owns that.

Every git project is tinted except the handful I'm always in, which stay bare so a plain
modeline means home turf.  A colour is reclaimed once its project has no buffers left,
lazily on the next allocation, so there's no ~kill-buffer~ hook and no timer; pin a project
when it has a colour of its own and the shuffle would be wrong.

The tint keys off ~default-directory~, not the file, so everything anchored on a project
wears its colour: the [[*agent-shell][agent-shell]] session, dired, magit.  Claude's worktrees are projects like
any other and get their own colour, which is the point — an agent window and the code it is
editing are the same colour or they are not the same tree.
...
Narrowing and heading navigation should feel the same whether I'm writing org or editing
code, so these commands dispatch on the major mode instead of being bound per mode.  The
capital variants narrow an indirect clone in another window, which lets me hold two
sections of the same file open at once.
...
An agent writes most of the config now, so ~cC~ lands on the diff to review rather than on the file.
...
[[https://github.com/xenodium/agent-shell][agent-shell]] talks to the real Claude Code agent over ACP and renders it into a native Emacs
buffer, so evil works and the buffer stays put — the TUI-in-a-terminal that [[https://github.com/manzaltu/claude-code-ide.el][claude-code-ide]] ran
could never manage either.  Provider and agent selection are [[file:~/git/konfig/readme.org::*agent-shell][work-specific]] and live in konfig.

The ACP adapter is a node program (=npm i -g @agentclientprotocol/claude-agent-acp=), so nvm's
=bin= has to be on [[*Homebrew PATH][exec-path]] or agent-shell won't find it.

There's no shift-tab: permission modes are ACP session modes here, cycled with =C-<tab>= or
picked by name with =SPC a M=.

Expandable sections carry their =RET= on a =keymap= text property, which evil's normal state
shadows, so =RET= submits instead of expanding.  Rather than fight that, the same motions as
[[id:b675fb83-50bf-4177-9082-f9c039befbb9][org keybindings]]: =C-j= / =C-k= walk items and =TAB= expands the one at point.

=gx= opens the link at point, since agent-shell ships a copy-the-URL command but no open-it
one, and =C-c C-o= here is "other buffer", not org's follow-link.  evil-collection owns the
rest of the =g= map (=gt= transcript, =gc= compose, =gm= mode, =gv= model, =gr= reload, =gy= session
id), so only =gx= is taken back — it means "open the URL under the cursor" everywhere else.

=+= / =-= / =0= resize the image under the cursor.  agent-shell already binds them; evil's
motions shadow them, and off an image they'd resize every image in the buffer, so they're
scoped to an image being under point and fall back to the motions everywhere else.

They go to agent-shell's markdown commands rather than its shell ones.  The shell commands
self-insert instead of resizing when point sits at the last prompt, so =+= can be typed into
a message being composed — but an image I've just attached is at the last prompt too, and
there =+= typed a =+= in front of it.  Scrolled up to an earlier image it resized fine, which
is why this looked intermittent.  The binding only fires with an image under point, so the
is-the-user-typing question is already answered and the guard is only ever wrong here.

A conversation is never unsaved, but every reply modifies the buffer, so [[*doom-modeline][doom-modeline]] paints
the name red and I reach for =C-x C-s= — the tell is wrong, not the reflex, so drop the
highlight here and point =C-x C-s= at the transcript, which is the thing there is to save.
...
=agent-shell-switch-buffer= already ranks every session with a coloured =agent-shell-status= —
=busy=, =blocked=, =ready= — but nothing tells me a session wants me while I'm looking elsewhere,
and there is no hook to hang that off, so poll instead.

Two things count as wanting me: =blocked=, which is a permission dialog nobody can answer but me,
and a =busy= → =ready= edge, which is a turn that just finished.  The first is a live fact and is
recomputed every time; the second is an event, so it sticks on the buffer until I actually look at
it.  =SPC a SPC= walks the waiting ones and falls back to plain cycling once none are.
...
The [[*waiting sessions][modeline tag]] counts the shells that want me, which is no help when three are cooking and
only one of them is the one I am actually waiting on.  =SPC a N= marks the shell at point, and
that one puts a [[*knockknock][notification]] in the corner when it stops, saying which session it was.

agent-shell's own author wrote this, so it hangs off the real ~turn-complete~ and
~permission-request~ events rather than my one-second poll guessing from the status, and the icon
is whichever agent the shell is actually running.  It skips a shell that is on screen in any
frame, viewport included: I saw it.  Which also makes watching the shell the one reliable way to
convince myself this is broken.

No ~:hook~, because every session shouting is the thing the modeline tag already does badly.
=SPC a N= turns the mode on for the one I am waiting on.

Both handlers are overridden, because upstream titles every popup =Finished= and leaves the
session it came from nowhere on it, which is the one thing I opened a notification to learn.  The
session name goes in the title and the status drops to the message; the agent prefix comes off
because the icon already says Claude.

Dropping ~:duration~ is what stops them expiring — it falls back to the [[*knockknock][global]] day, while
~agent-shell-knockknock-duration~ goes on timing the transient =RET= that jumps to the shell.  The
two only looked inseparable because upstream passes the one variable to both.

Each one carries the shell it came from as its [[*knockknock][:action]], so =SPC X= goes there without the ten
seconds the transient =RET= gives me.  Six sessions open is the case this is all for, and the
icon stays the plain white alien: tinting it per project read as mud against the dark popup.
...
*** agent-shell consult source ~a~

Sessions are just buffers, so they already turn up in ~consult-buffer~ — but mixed in with
everything else, which is the "which session is which" problem again.  Their own source
narrows to them with =a=, ordered by recency the way ~agent-shell-buffers~ returns them.

Annotating the source with the same status gets ~consult-buffer~'s =a= narrowing to the same
information ~agent-shell-switch-buffer~ shows.
...
*** switch-buffer preview

~agent-shell-switch-buffer~ already renders what I want — icon, name, coloured status, session
title, aligned — it just has no preview.  Rather than rebuild that in a consult command, hand
agent-shell's own collection to ~consult--read~: it wraps any completion table, and merges the
table's metadata, so the affixation function keeps painting the icons and the candidates stay
byte-for-byte the ones agent-shell built.

Preview needs a candidate → buffer map, which agent-shell keeps to itself, so pass =:buffers=
explicitly and pair it positionally with ~all-completions~.  That list is also where sessions
[[*waiting sessions][waiting on me]] get floated to the top, since the point of looking is usually to find them.

Only ~consult--buffer-preview~, not ~consult--buffer-state~: selecting is agent-shell's job.
...
The session picker lists every conversation I have ever had, which is no help finding the three
that were actually open when I shut down — so record those at =kill-emacs=, and =SPC a L= resumes
all of them by session id the next morning.

Oldest first, so the [[*switch-buffer preview][picker's]] recency order survives the night, and none of them displayed,
since picking between them is that command's job.  An Emacs with no shells open has nothing to
say about where I left off, so it leaves the record alone rather than emptying it.
...
~session/list~ takes a cwd, so agent-shell's picker only ever answers "what did I run here".
=SPC a H= is the same picker over every directory at once, read out of Claude Code's own store —
a transcript per session under =~/.claude/projects/= — and resuming the pick where it was running.

This is what a crash leaves me with: =SPC a L= has nothing to replay, because =kill-emacs-hook=
never ran.  Headless runs would crowd the list out, so cwds under =~/.cache= (the
[[file:~/git/konfig/readme.org::*Leak check][leak check]], claude-auto) and the =/=-rooted cron are dropped.
...
A task that opens a shell in a worktree always opens it the same way, and the =/boot= that
follows is part of the task rather than something to retype.  ~ads/agent-shell-kickoff~ is the
block those tasks already carry, with the prompt in it:

#+begin_src emacs-lisp :tangle no
(ads/agent-shell-kickoff "/boot" "~/git/pr/multi-burn/")
#+end_src

The directory is the whole point, so nothing about the shell may be inferred from where I
called this.  ~agent-shell~ itself is DWIM — from a shell buffer it toggles that window and
opens nothing, and its =prompt= [[*agent-shell][session strategy]] would sit on a picker — so start the shell
outright, on a new session, and hold onto the buffer rather than looking it up again later.
An existing shell in that directory is reused: two agents on one worktree is the one
arrangement guaranteed to waste both of them.

The target directory is only ever set in a temp buffer, never bound around the lookup.
~default-directory~ is buffer-local, so letting it in a shell buffer rewrites *that shell's*
cwd for the duration of the call — and since finding the shell to reuse means reading cwds
back off buffers, the shell I called from answers to the directory I asked for and takes the
prompt itself.  Which is how this one sent =/boot= to the session that ran it.

agent-shell will hold the prompt for me.  ~agent-shell-prompt-queue~ sends it now if the
session is up, queues it behind the current turn if one is running, and otherwise hands it to
agent-shell's own =prompt-ready= subscription, which fires when the session finishes
negotiating — a poll for the same moment is a poll that can be wrong.
...
#+end_src
** apheleia
[[https://github.com/radian-software/apheleia][apheleia]] runs code formatters on save, asynchronously and without moving point.  JSON is formatted
with =jq= (=prettier= isn't installed, and =jq= ships everywhere).  The =jq '.'= identity filter reindents
while preserving key order; it also rejects invalid JSON, so a broken file simply isn't reformatted
until you fix it.  Elisp uses apheleia's built-in =lisp-indent= formatter (Emacs' own indentation
engine, no external tool), which is already the default mapping for =emacs-lisp-mode=, so those hooks
need no =setf=.

Opt in one language at a time: add its mode to =:hook=, and (if the default formatter isn't installed)
point =apheleia-mode-alist= at one that is.

Rust uses the nightly =rustfmt= binary directly (some of our options are unstable and stable
rustfmt ignores them; the rustup =+nightly= shim isn't honored in apheleia's environment) against
a tangled =~/.config/rustfmt/rustfmt.toml=.  The binary is globbed out of =~/.rustup= rather than
named, so this doesn't hardcode a host triple.

That =--config-path= is only passed when the project doesn't ship its own =rustfmt.toml=.  rustfmt
reading from stdin resolves =rustfmt.toml= relative to its working directory, so a project's own
style wins on its own — but only if we don't override it.  Any element of an
=apheleia-formatters= entry that isn't a string is evaluated at format time and spliced in, so
returning nil from the helper simply drops both arguments.
#+begin_src emacs-lisp
...
#+end_src
** auctex LaTeX                                                   :external:
[[https://www.gnu.org/software/auctex/][AUCTeX]] for editing standalone =.tex= files.  Note that inline org previews do *not* go through
AUCTeX -- see [[*org latex preview][org latex preview]] for that.

Needs a TeX distribution: MacTeX on macOS, =texliveFull= on Linux, MiKTeX on Windows.
...
Notifications rather than polling, which is how "instant" stops costing anything.
...
** cdlatex
[[https://github.com/cdominik/cdlatex][cdlatex]] makes math fast to type: =_= and =^= auto-brace, and an environment name then TAB
expands a template.  Org binds =`= and ='= unconditionally (not just inside fragments), so unbind
them and type =\alpha= longhand.
...
auto-fill's hard breaks are right in the file and wrong everywhere I paste to, so both commands join
them back up on the way to the kill ring.  Region when there is one, whole buffer otherwise.

For markdown the export does the joining: neither markdown backend is in ~org-export-backends~, so
[[*ox-gfm][ox-gfm]] has to be required or it fails on a cold Emacs, and ~:with-toc nil~ drops both the table of
contents and the HTML anchor that otherwise lands above every heading.

Plain text goes through a backend derived from ~ascii~, which already unwraps paragraphs and draws
tables, but keeps =/emphasis/=, quotes ~code~, boxes src blocks in =,----= and underlines headlines
with a rule.  Overriding those handlers strips the markup and leaves the words; links become their
description, or the URL when they have none, and headlines keep org's own stars, which read better
than the underline.  ascii's section joining then leaves a stray blank line before each headline,
so runs of them collapse on the way out.

Outside org there is nothing to convert or strip, so both fall back to unfilling the text.
~fill-region~ with a huge ~fill-column~ is the obvious unfill and quietly flattens src blocks, tables
and drawers along with the prose, so in org only paragraph elements are rewritten.
...
** d2-mode                                                       :external:
[[https://github.com/andorsk/d2-mode][d2-mode]] for [[https://d2lang.com][D2]] -- the "just draw it for me" counterpart to TikZ: write =a -> b= and it
places everything.  Needs =brew install d2=.

There is no =ob-d2=; =d2-mode.el= defines =org-babel-execute:d2= itself, so d2 must *not* go in
=org-babel-do-load-languages=, and =:commands= is what autoloads the symbol babel dispatches on.

Never =.png= -- D2's PNG export shells out to Playwright and its driver download 404s.
...
*** making d2 blocks behave like latex previews

Replaces =org-babel-execute:d2= to fix three things:

- *false errors* -- =d2= logs success to stderr and has no =--quiet=, and =org-babel-eval= treats
  any stderr as failure; check the exit status instead.
- *opaque background* -- =style.fill: transparent= is appended (not prepended, so error line
  numbers still match) unless the block sets a root fill itself.
- *theme* -- =--dark-theme= embeds a =prefers-color-scheme= query that librsvg ignores, so the
  theme is picked at render time from the =default= face's luminance.

Any =:results= that is not =file= renders box-drawing text into the buffer instead of an SVG --
no image, no theme problem, and it diffs.  =<da= inserts such a block.
...
=C-c '= gives a real =d2-mode= buffer, but =d2-compile= reads =buffer-file-name= -- nil there,
so =C-c C-c= errors.  Fall back to the buffer, which is what =d2-compile-buffer= already does.
...
=:file-ext= and =:output-dir= let babel derive the filename from the block's =#+name=, so a
=#+name: pipeline= block needs no header args at all and writes =img/pipeline.svg= -- which is
what =<d= expands to.

=:output-dir= is prefixed onto an explicit =:file= too, so =:file ./custom/x.svg= lands at
=img/custom/x.svg=; use =:output-dir .= to escape.
...
Rewrites the image files only; re-executing the blocks properly would dirty the buffer on every
theme toggle for output that differs only in colour.
...
** default-text-scale
Scale text globally across all frames and faces.

Increase on J because as text scale increases the line your looking at goes down

=transient-define-prefix= is an un-autoloadable macro, so =:after transient= is what makes the
=:config= below run at all — alphabetically this lands ahead of every =(require 'transient)= in
the file, and without the =:after= the prefix silently fails to define.
...
Yazi in emacs: parents, listing and preview in three columns, ~SPC o D~ for the current directory
and ~SPC o d~ for org.

The three columns only come from the ~dirvish~ command: ~dirvish-override-dired-mode~ builds a
session with no layout, and no layout means there is no preview window at all.  ~dired~ and
~dired-jump~ are remapped onto it, and ~ads/dirvish-columns~ catches everything else that lands in
a directory — ~find-file~, ~C-x C-f~, a project switch — the moment the buffer reaches a window.

Dirvish means "full frame" by that: it runs ~delete-other-windows~ and measures its panes against
~frame-width~, so opening a directory throws away whatever else I had on screen.
~ads/dirvish-in-window~ scopes both down to the window I opened in.  The columns land a few
characters wider than the window they replaced, and ~q~ still puts everything back.

Dirvish only binds ~q~ itself and evil-collection has no dirvish module, so the normal state keys
below are the difference between dirvish working and dirvish doing nothing.  ~q~ has to be dirvish's
own to restore the window configuration the columns replaced.

Directories preview through ~ads-dir~ rather than dirvish's own, which shells out to ~emacs -Q~ and
gets back a bare ~ls -l~ dump with none of my settings in it.  Drawing them the way the parent
columns are drawn means one listing that looks the same in all three places, at the cost of leaning
on two of dirvish's internals.

Icons are dirvish's own attribute rather than [[*nerd-icons][nerd-icons-dired]], which can't reach
the parent columns.  Text, pdf and fonts preview as is; image, video, audio and archive want
~vips~, ~ffmpegthumbnailer~, ~mediainfo~ and ~sevenzip~ on PATH, and dirvish doesn't fall back to
text when they're missing, it just previews nothing.
...
** ediff
built diffing packag, seems pretty good
...
** eglot
emacs batteries included LSP mode - disabled in favor of lsp-mode
...
** flycheck
[[https://www.flycheck.org/][flycheck]] for on-the-fly syntax checking. Enabled only in JSON buffers for now,
where its built-in =json-jq= checker uses the =jq= binary to flag malformed JSON
as you edit. =lsp-mode= will also prefer flycheck over flymake when it's present.
#+begin_src emacs-lisp
...
like vterm but using ghotty as a backend insted.  This is newer and much more experimentla but in
breif testing this feels much smoother and faster than vterm.  The point is to stop opening
Ghostty.app at all.

Keys fall through to zsh in the default semi-char mode, which is the only way =TAB= is the
completion I actually have — line mode holds the input in Emacs and completes it against
comint's idea of a shell instead.  evil-ghostel is what makes that liveable, translating =dw= and
friends into what zsh's line editor wants.  =C-c C-l= borrows line mode for one buffer when a
command wants real editing.

New terminals open in normal state, which is the reflex everywhere else.  ESC still reaches
alt-screen apps, so =q= quits =htop= without ever leaving insert.
...
The terminal I always want is one key away and never a buffer I have to find: =SPC ;= drops it
across the bottom of the frame and the same key puts it away.  Only the window goes — the shell
and its scrollback are still there next time.

=ghostel= keys a terminal by an identity built from the buffer name it was started under, not
by the name the buffer currently has, so a wrapper that binds =ghostel-buffer-name= gets a slot
of its own and =SPC s n= can still open ordinary shells beside it.  That indirection is what lets
[[*session names][session names]] rename these buffers afterwards without ghostel losing track of the slot.

Finding the popup again is the one thing the rename does cost.  The toggle and the side-window
rule both used to match the name exactly; both now match the =ghostel-popup= prefix, under an
optional star — the creation name is =*ghostel-popup*= and the renamed form has no stars at all,
so the prefix is the only part the two share.
...
elsewhere, and starts it the first time."
  ...
  (let* ((buffer (ads/ghostel-popup-buffer))
    ...
    (cond
     ((eq window (selected-window)) (delete-window window))
...
Four terminals all called =*ghostel*= are four terminals I have to open to tell apart.  A name
worth reading carries the host when it is not this machine, and where the session is sitting.

The stars go with them.  =*foo*= is Emacs' convention for a buffer with nothing behind it, and a
shell sitting in a directory is not that — these read as =ghostel: konfig=, bare.  They are born
=*ghostel*= all the same, because that name is what ghostel builds the slot identity from.

Local directories get the git root and both ends of the path under it —
=~/git/gravitas/sim/scenarios/scenarios/gnc/= reads =gravitas/sim/.../gnc=.  The middle of a deep
path is never the part I am identifying it by, and the two ends plus the repo are.  Outside a
repo there is no root to anchor to, so the path stays whole.

Remote sessions lead with the host and stop there: =arches:~=.  No git lookup runs on them,
because =locate-dominating-file= over TRAMP is a round trip per parent directory and the
package goes out of its way to keep synchronous TRAMP calls off this path.

Nothing here sets a terminal title.  ghostel's zsh integration emits OSC 7 and the prompt marks
and never OSC 0/2, and there is no title hook in my =.zshrc= — so the TITLE argument is ignored
rather than guessed at.  This is the function that would use one if that ever changes.

Two calls, not one, because the two renaming paths have different blind spots:

- =ghostel-buffer-name-function= fires on every =cd= and every title change, which is what keeps
  the name honest as I move around.  It fires on /every prompt/, though, so the git lookup is
  skipped whenever the directory has not actually moved.
- =ghostel-pre-spawn-hook= names the terminal as it is born.  Without it a remote session would
  never be renamed at all: =ghostel-tramp-shell-integration= is nil, so no OSC 7 ever arrives
  from =arches= and the buffer would keep the bare name for life.  It renames through ghostel's
  own =ghostel--rename-managed= — private, but reaching past it would leave the package's rename
  bookkeeping stale and silently kill the =cd= tracking above.

Both decline on anything that is not a plain terminal: =ghostel-exec= and =ghostel-compile= put
buffers of their own into =ghostel-mode=, and those keep the names they arrived with.
...
Terminals are buffers too, so they get the same source treatment the agent shells do: =t= narrows
~consult-buffer~ to them, in the recency order =buffer-list= is already in, and previews each one as
I move down the list.

Their buffer names are all much the same, so the annotation is the directory each terminal is
sitting in, which is what I am really picking by.

~consult-buffer~ takes its sources as an argument, so the same source is also the whole of
=SPC s b=, in place of ~ghostel-list-buffers~.
...
** GPS time conversion                                             :custom:
Mostly vibe coded but very helpful since I deal with waaaay too many instances of GPS time which
makes me sad.  No human should have to read any form of GPS time and be expected to understand it.
GPS time is cursed, who the fuck wants seconds per week???

#+begin_quote
GPS time counts monotomically up from the GPS epoch (1980-01-06 00:00:00 UTC) and, unlike UTC, does
NOT reinsert leap seconds.  Unix time also skips leap seconds, so the running difference between GPS
seconds and (unix - gps-epoch) is exactly the accumulated leap-second count (18 as of 2017).  This
is critical for real time systems particularly in space where a monotonic clock is non-negotiable
for time keeping.

These functions apply that offset via a leap-second table.
#+end_quote
...
** ident-bars
[[https://github.com/jdtsmith/indent-bars][Github - Ident Bars]]

Use a stipple to show indentation, except in the hidden buffers org uses to
fontify src blocks, since those faces leak into the org buffer and look awful.
...
[[https://github.com/minad/jinx][jinx]] spell checks the visible window only, through a C module talking to libenchant, so it stays out
of the way in a long org file the way flyspell never did.  Needs =brew install enchant=, which pulls
aspell and its dictionaries; the module builds itself on first use given =pkg-config= and a compiler.

~jinx-languages~ defaults to whatever the locale says, and GUI Emacs on macOS inherits no =LANG=, so pin
it rather than let it ask enchant for a "UTF-8" dictionary.

Squiggles under everything I type are a distraction, so nothing is on by default.  =M-$= sweeps the
buffer instead: ~jinx-correct-all~ walks every misspelling in turn, numbering the suggestions =1= to =9=
and taking =M-n= / =M-p= to skip one without fixing it.  It switches ~jinx-mode~ on to do that and
leaves it on, hence the wrapper putting it back.  =SPC t j= is there for when I do want live checking.
Code faces are already excluded for org and markdown upstream, so only prose gets checked.
...
A posframe in the top right of every frame, which is the one way anything in here interrupts me:
[[*notify when a session finishes][agent shells]] today, meetings and the [[file:~/git/konfig/readme.org::*Claude Bot inbox loop][inbox loop]] later.  =SPC x= puts one away, =SPC X= goes to
whatever sent it.

Nothing of mine auto-dismisses.  Emacs is always on screen but not always the focused app, and a
banner that times out while I am in a browser is a banner I never saw.

One popup per notification, stacked downwards from the corner with the oldest on top, because a
second agent finishing must not quietly overwrite the first.  Both keys take the oldest, so the
one I act on is always the one in the corner and the rest close up towards it.

One popup per frame as well, each in its own buffer named after a frame parameter.  posframe
parents its child frame to ~(selected-window)~, which for a background event is the last frame I
touched rather than the one I am looking at — that is how a finished session ended up announcing
itself over Slack on the monitor I never read.

Rendered through ~knockknock--notify-internal~ rather than ~knockknock-notify~, because the public
entry point debounces on a 0.15s timer that keeps only the last call and fires long after the
per-frame bindings have unwound, collapsing the whole loop into one popup on an arbitrary frame.

The colour runs through the border, the background, the title and the icon, which is the look
knockknock shows off in its own screenshots.  Green for [[*notify when a session finishes][agent shells]], blue for [[*org meetings][meetings]]; each
source passes its own, and anything it leaves out falls back to the global.

The background has to be painted into the SVG, not just handed to posframe.  knockknock draws
icon and text over nothing, so every surface the image misses shows through in the theme's own
colour — the fringes, which take the ~fringe~ face rather than the frame's background, and the
part of a character cell ~fit-frame-to-buffer~ rounds up to.  Those were black bars around a blue
popup.  Filling the canvas, matching the frame and zeroing the fringes leaves nothing to show.

Anything painted into the image has to be in the SVG cache key, which upstream builds from the
text and icon alone.  Two sources sharing a title and a message would otherwise serve each
other's colours.

The agent's logo is recoloured on disk before knockknock sees it, because ~svg-embed~ puts a PNG
in untinted and only nerd-font glyphs get filled with a face.  It is shipped white, which is
invisible the moment I am in a light theme, so it is tinted to the foreground rather than to
anything decorative — per-project colours were the first idea and read as mud.

Title and icon inherit ~success~ and so follow a theme toggle on their own, but
~knockknock-border-color~ is a colour string rather than a face, so it is re-synced from the
same [[*modus-tweaks][hook]] that carries everything else across.

Tracking [[https://github.com/konrad1977/knockknock/pull/4][xenodium's fork]] rather than =konrad1977/knockknock=, because that is the one where
~:icon-file~ handles a PNG, which is what [[*notify when a session finishes][agent-shell-knockknock]] hands it.  Back to upstream
once the PR lands.

Positioned by hand rather than with ~posframe-poshandler-frame-top-right-corner~, which asks for
=-1= and leaves the negative to ~set-frame-position~ — a child frame on macOS reads that as zero
and parks in the top *left*.  Subtracting the width myself keeps the number positive.

The same handler pulls a popup back onto the monitor when the frame it hangs off runs past the
edge, since ~posframe--set-frame-position~ skips any move whose coordinates have not changed and a
bad placement would otherwise stay put.
...
** ligature
[[https://github.com/mickeynp/ligature.el][ligature.el]] - programs ~composition-function-table~ so FiraCode's ligatures actually render.

~:noweb no~ is required here: the inherited ~:noweb yes~ treats the ~"<<" ">>"~ pair in the ligature
list as a noweb reference and expands it to the empty string.

Org body text is [[*org][variable-pitch]] EtBembo, which has no ~calt~ table and so cannot ligate; putting
just these sequences in ~fixed-pitch~ hands them to FiraCode while the prose stays serif.  The one
list drives both halves: ligature.el composes the sequence, the face picks the font that has the
glyph, and a sequence missing from either one silently does nothing.
...
** lsp-mode
[[https://emacs-lsp.github.io/lsp-mode/][lsp-mode]] - Language Server Protocol client for Emacs

=lsp-diagnostics-provider= is pinned rather than left at =:auto=.  =:auto= picks flycheck if the
feature happens to be loaded and flymake otherwise, and since [[*flycheck][flycheck]] is =:hook=-deferred to JSON
buffers, the backend used in a Rust buffer would otherwise depend on whether you'd opened a JSON
file first that session.

=lsp-file-watch-threshold= is raised because the default of 1000 files trips on any monorepo, and
tripping it means a =y-or-n-p= prompt every time a workspace starts.  The servers that matter here
are told to do their own watching instead (see [[*lsp-rust][lsp-rust]]), so this only governs the ones that
aren't.

Raising the threshold alone isn't enough, though, because the count itself is wrong in a Bazel
repo.  lsp-mode ships patterns for =bazel-*=, but Bazel also drops a convenience symlink plainly
named =external= that points into the output base's external tree, and no default pattern catches
it.  Following it took the watchable-directory count in kode from 2,861 to 67,191 — the extra 64k
being the Bazel repository cache — so the prompt fired no matter how high the threshold went.

Code action, format and find-references sit on capitals rather than the obvious =la= / =lf= / =lr=.
On the work machine [[file:~/git/konfig/readme.org][konfig]] claims those three under general.el's =override= keymap, which lives in
=emulation-mode-map-alists= and so outranks =lsp-mode-map=; bound lowercase they would silently do
nothing there.  Capitals keep one set of keys that works on every machine.

Autostart goes through =ads/lsp-maybe-deferred= rather than =lsp-deferred= directly, so a project
can opt out of having a server started for it just by opening a file.  The default is an empty
predicate list, which is plain =lsp-deferred=; the reason it exists is that a server is not always
cheap enough to start on a whim.  rust-analyzer on a large Bazel graph holds several gigabytes and
takes a minute and a half to prime, so on [[file:~/git/konfig/readme.org][the work machine]] kode claims manual control and starts it
on a keybinding instead.  Deciding that per project belongs to the project, not here, hence the
hook.  =run-hook-with-args-until-success= means any predicate returning non-nil suppresses the
start - explicit =M-x lsp= is unaffected either way.
...
** lsp-rust
[[https://rust-analyzer.github.io/][rust-analyzer]] configuration for lsp-mode.

The server binary comes from =PATH=.  Under rustup that's a shim which reads the project's
=rust-toolchain.toml=, so the analyzer always matches the compiler the project pins — and the
config keeps working on Linux and Windows, which hardcoding a =*-aarch64-apple-darwin= path did
not.

Only one behaviour setting is general enough to belong here.  =use-client-watching= nil hands file
watching to rust-analyzer's own notify backend instead of registering thousands of directories with
Emacs' =filenotify=, which costs one kqueue fd each on macOS.

What a Bazel repo needs on top — proc macros off, build scripts off, =cargo check= off, and the
diagnostic codes that unexpanded macros make untrustworthy suppressed — is set per buffer by [[file:~/git/konfig/readme.org::*Grove - rust-analyzer][the work
config]] instead of globally.  Those settings are actively wrong for a Cargo project: they throw away
proc-macro expansion, which Cargo builds for the host and which therefore works fine, along with the
on-save diagnostics Cargo gives you for free.  Setting them here broke every Cargo crate on the
machine in order to fix one monorepo.  Worth remembering while you are in the area that the on-save
knob is =lsp-rust-analyzer-cargo-watch-enable= and *not* =lsp-rust-analyzer-check-on-save=, which
looks plausible but is not a variable lsp-mode defines.
...
** lsp-pyright
[[https://github.com/emacs-lsp/lsp-pyright][lsp-pyright]] fronts either pyright or the [[https://docs.basedpyright.com/][basedpyright]] fork.  basedpyright, installed with
=uv tool install basedpyright=, so there's no npm in the loop and no per-project =node_modules=.

The client has to be registered *before* [[*lsp-mode][lsp-mode]]'s own =python-ts-mode= hook fires, or
=lsp-deferred= finds no client that handles the buffer and gives up, hence the explicitly-earlier
hook depth rather than =:hook=.
...
Magit's HEAD build calls llama's ~any~/~all~ helpers, but MELPA's published llama
doesn't include them yet.  These shims tide it over.  Loading llama first means
the guards skip automatically once MELPA ships a llama that defines them, so this
block is safe to delete then.
...
#+end_src
** markdown
Markdown should read and navigate like org, so the markup is hidden, the headings scale and the
same keys work.  =markdown-ts-mode= is built in as of Emacs 31 and replaces =markdown-mode=
everywhere; =markdown-ts-mode-x= adds its conversion/preview and table-of-contents commands.  The
=markdown= and =markdown-inline= grammars install themselves on first use.
...
The same motions as [[id:b675fb83-50bf-4177-9082-f9c039befbb9][org keybindings]]: =C-j= / =C-k= walk visible headings, =TAB= cycles and
=RET= follows the link at point.
...
#+end_src
*** org-todo-keywords
I used to have more here, but I've slowly realized that a more complex todo flow is just something
that I don't usually use.  additonal states are here for SW dev where there's a frequent back and
forth on tickets
...
*** resize the image at point

The same =+= / =-= / =0= as in [[*agent-shell][agent-shell]], scoped the same way so they stay evil motions
everywhere else.  Unlike a chat log an org file is a document I keep, so a resize writes
=#+ATTR_ORG: :width= above the link rather than tweaking the overlay: it survives a redisplay,
a reopen and a git clone.  Widths stay a fraction of the text width, so they still reflow.
...
*** org weekly review                                             :custom:
I do a weekly review, this will create the weekly review file in my current
daily and fill it in with some basic information.  Active projects and this
month's goals come in with the template so there's nothing left to call by hand.
...
of everything that is there.  The weekly reads the Goals heading back out of
here by roam title rather than path, since on work machines the note gets
refiled under =personal/=.
...
*** Task Frame
Spin up a dedicated and stripped down aenda view to have up in the background
...
An agenda line is painted once and ~org-agenda-sticky~ keeps the buffer across a toggle, so
the colours baked into it are the old theme's — white on white going light — and no amount of
[[*force reload][reloading the theme]] reaches text that has already been written.
...
Convenience package that makes org bulleted list behave like they do in most other apps
...
=d2= needs its own template rather than a list entry: the block wants a =#+name:= line *above*
=#+begin_src=, and =org-structure-template-alist= can only fill in text after it.
...
=latex= enables =#+begin_src latex :file diagram.svg= blocks, which write TikZ/pgfplots out to
image files on disk; see [[*org latex preview][org latex preview]] for the inline case.  =dot= is Graphviz, and ships with
org.  =d2= is deliberately absent -- it has no =ob-d2= feature to require, so listing it here
would error.

Babel evaluates elisp with =:lexical no=, but every block in this file is =init.el=, which is
lexical -- so a =C-c C-c= here quietly redefines a function with dynamic binding and its
closures go void somewhere else entirely, in a timer or a hook, long after the block ran.
...
The title reuses the alias with =%\1= rather than asking for the short form twice.  The
backslash is doubled in the source so the Lisp reader does not take =\1= for an octal escape.
...
** org-fragtog
[[https://github.com/io12/org-fragtog][org-fragtog]] previews the LaTeX fragment the cursor is in and un-previews it when you move in to
edit, which is what makes [[*org latex preview][org latex preview]] feel live rather than a manual toggle.
...
** org latex preview                                               :custom:
Inline rendering of math *and* TikZ pictures directly in the org buffer -- plots, circuits,
flowcharts, state machines, trees.  [[*org-fragtog][org-fragtog]] toggles these automatically as the cursor moves;
[[*cdlatex][cdlatex]] makes them fast to type.

*** Drawing preamble

One package list shared by both rendering paths below, so a picture that previews inline renders
identically when written to a file: =pgfplots= plots, =circuitikz= circuits, =forest= trees,
=tikz-cd= arrow-heavy graphs, and the =automata= library for state machines.
...
=dvisvgm= over the default =dvipng= because SVG stays sharp at any text scale; the stock alist
entry is already correct, so it's selected rather than redefined.

Appending to =org-format-latex-header= is what makes non-math work -- out of the box a bare
=tikzpicture= fails on an undefined environment.

=:background "Transparent"= is not cosmetic: the default bakes a =\pagecolor= rect into the SVG
that Emacs cannot recolour, which under a dark theme renders every fragment as a white slab.

=:scale= multiplies the =:image-size-adjust (1.7 . 1.5)= the =dvisvgm= entry already carries, so
1.4 is an effective 2.38x -- deliberate, and vector output so nothing blurs.
...
Two ob-latex defaults have to go: =org-babel-latex-preamble= ships
=\def\pgfsysdriver{pgfsys-tex4ht.def}=, which points TikZ at the tex4ht backend and is why =latex=
blocks silently produce a broken PDF, and the SVG step hardcodes =inkscape= where =pdftocairo=
from poppler does the same job.
...
*** regenerate previews on theme change

Previews bake the foreground colour in at render time, so [[*Toggle themes][Toggle themes]] leaves them unreadable.

Rendering is synchronous and costs about a second per fragment, so this runs off an idle timer
(the theme paints first) and does each window's visible region before the rest of the buffer.
=org--latex-preview-region= is internal but the only entry point that takes bounds.
...
The two wrappers just hide =org-latex-preview='s =C-u C-u= / =C-u C-u C-u= prefix args.  =ov=
rather than the obvious =op= because the work config binds =op= globally to
=ads/refile-move-personal=, which a prefix in =org-mode-map= would shadow.
...
Three minutes before anything in the agenda with a time on it, through the same [[*knockknock][posframe]] the
agent shells use.  =SPC X= opens the heading it came from.

Blue outlines the lot, so a glance says agenda rather than agent.  A =:meeting:= also gets the
filled background and a blue icon, because a meeting is the one I have to be somewhere for; a
plain TODO is the outline and nothing else.

~appt~ does the work, over scheduled, deadline and plain timestamps that carry a time.  An
~APPT_WARNTIME~ property overrides the lead per entry for free.  Its beep and its ten-second
teardown are both off, because [[*knockknock][nothing of mine auto-dismisses]].

~appt-display-interval~ sits one above the warning time so the reminder fires once at T-3 rather
than again at T-0.

Rebuilt daily at 00:05, which is not optional: appt keeps a time as minutes after midnight with
no date on it and only prunes what is already past, so without it today's entries read as
tomorrow's and fire again at the same clock time while tomorrow's are never added at all.
Rebuilt on every agenda build too, so rescheduling something takes effect without waiting, and
once at the end of this block — that one finds nothing, since ~org-agenda-files~ is not claimed
until ~after-init-hook~, but the 00:05 timer is already overdue by then and fires on the first
run of the timer list, which is after init.  Cheap enough to leave both.

The icon is the glyph the [[*roam-categories][category]] already starts with, so a notification looks like its line in
the agenda.

~org-todo-regexp~ is no use for trimming the keyword off the title: it is buffer-local and nil
everywhere but an org buffer, and appt calls from wherever I happen to be.  Built from
~org-todo-keywords~ instead, which also keeps a heading like =PPU1 Anomaly Brief= intact where
eating a leading capitalised word would not.

Where the entry lives is kept as a file and a position rather than a marker, because
~org-agenda-to-appt~ ends by releasing every buffer it opened to do the scan.
...
** org-modern
Org-modern shrinks a table separator row to ~org-modern-table-horizontal~, but org-indent's
full-height ~line-prefix~ pins the row back to a whole line, so hlines are thin at root and fat
under a heading; the advice re-does that prefix at the same pixel width and the hline's height.
...
** org-noter
[[https://github.com/org-noter/org-noter][org-noter]] pairs a PDF with an org file so notes are anchored to the page (or the exact highlight)
they came from.

The workflow is roam node → =C-c C-a a= to attach the PDF → ~SPC o n~, which offers the
attachments and writes the chosen one into a =:NOTER_DOCUMENT:= property relative to the node file.
Notes then live in the node itself; =i= takes one on the current page, =M-i= on the selection.

=org-noter-supported-modes= gates which backend modules get required, and the djvu one nags about
a missing =djvu= package on every load, so it's dropped.
...
For my [[weekly review]] I want a list of active projects, the weekly template
fills itself in with ~ads/roam-active-projects~ so I never have to call it.
...
came from adding an ~:ARCHIVE_NODE:~ property.  Additionally move the attached data if there
is any
...
a node immediately and not dive further into it, tagged =:stub:= so I can find
what I still owe myself later.  Credit to [[https://www.youtube.com/watch?v=AyhPmypHDEw&lc=Ugw7bYrPOc6oy_UBmPZ4AaABAg][Umar Ahmad]]
...
*** roam-modeline
Show the ~#+title~ in the modeline instead of the timestamped roam file name.
The title lives above any narrowing, so the search has to widen first — otherwise
narrowing past the search bound throws and the modeline loses the name entirely.
...
*** roam-project-complete
Finishing a project moves its tag from =:project_a:= or =:project_h:= to
=:project_c:= and stamps the file node with a =COMPLETED= date.  The
celebration comes from =brew install gradigit/tap/confetti=, and is skipped on
...
#+end_src
*** roam-refile-category
Refile targets are whole files, and a file's title alone is often not what I
call it.  Show the =:CATEGORY:= instead, icon and all, so the picker both reads
and matches the way I think of the note.
...
have to move a file somewhere else and there are attachments, this function
...
*** roam-stub-tag
Any roam file that is nothing but its header gets tagged =:stub:= on save, and
loses the tag as soon as it has content.
...
** org-inbox-review                                                :custom:

Lightweight SRS-style review for inbox TODOs. Cycles through items and lets you snooze (schedule for later), complete, delete, or refile them.

Heavily done with claude code
...
#+end_src
** org-timegrid
[[https://github.com/Gleek/org-timegrid][org-timegrid]] draws an SVG week calendar out of the timed timestamps already in
~org-agenda-files~ and writes drags and resizes back to the heading they came from.  =SPC o g=
opens it; it reads ~calendar-week-start-day~, so the week starts on [[*org][Sunday]] like everything else.

The grid is a ~special-mode~ buffer with a command on nearly every bare letter, all of which
evil's normal state owns, so it opens in emacs state: =q= quits, =C-z= gets evil back, and =?=
prints the whole map, my rebindings included.

Nobody has written the evil layer — evil-collection has no module for it and there is no
package — so it is the one below, in the mode map rather than an evil state, since emacs state
is what keeps the package's own control keys working.  =hjkl= walk the cursor and =C-j= =C-k= do it
an hour at a time; =HJKL= move the block under it and =C-S-j= =C-S-k= move it by the hour; =<= =>=
drag its start and =[= =]= its end, left key earlier and right key later in both pairs, so moving
a block and resizing it never share a key; =n= =N= step between blocks and =G= asks for a date.  =v= starts
a selection, because =C-SPC= is the [[*General.el][leader]] in every state and would never reach the calendar.
For the same reason =C-g= is bound here explicitly: in emacs state it is ~ads/keyboard-quit-dwim~,
which answers it by leaving emacs state, and that unbinds everything above.

=RET= only ever opens the Org heading behind the block — the package also makes an entry with it
when the slot is empty, which is a lot of surprise for a key that otherwise means /show me
this/.  Making things is =o=: at the cursor, prompting with completion over every unfinished
TODO in the agenda files, writing the range onto the one I pick and capturing a new entry when
what I typed matches nothing.  =v= and a motion first gives it a length rather than the default
half hour, which is the way to fill an empty slot: =v=, =j= down to the end of it, =o=, then pick the
task off the completion list.  =a= asks the same question the other way round, task first and then when, and
=SPC o G= is that same command from outside the calendar.

A timed block is clamped to the visible week, so the built-in move stops dead at Saturday
rather than kicking something into Monday.  =H= and =L= fall back to writing the new range
straight to the backend when they hit that edge, and the view follows the block into the next
week.

Agenda files are only recomputed when [[*org-roam][the agenda opens]], so the week takes the same =:before=
advice — without it a roam file tagged =:agenda:= this session is missing from the grid.

Blocks dragged into empty space capture to [[*org-capture][inbox.org]] rather than the package's own
=calendar.org= in =.emacs.d=, so a meeting invented on the calendar lands where every other
capture does.  Saved on write, because that file is open and edited elsewhere.

The day strip rides on ~org-agenda-finalize-hook~ and sits at the top of the buffer.  Its
default anchor is a block named "To Refile", which I don't have, and the name is matched as a
regexp against the whole agenda — an item that happened to say those words would drag the strip
down to it.  =RET= with point on the image opens that day in the week view.

It draws first on that hook, ahead of ~org-modern-agenda~, so it draws inside
~with-demoted-errors~: once it took org-modern down with it and the agenda rendered as plain
text.  A failed open of the week view is cleared the same way — the package builds the buffer
before it loads the week, so an error in the load leaves a stateless buffer that every later
open trips over, and a =:before= advice kills it rather than wedging the calendar for the session.

The canvas is 8:00 to 22:00, which is the day I actually keep.  Anything timed outside it is
off the canvas rather than squeezed onto it, so =-= and === narrow and widen the day by an hour
at each end, =0= cycles waking / working / the whole 24, and =w= drops the weekend for a Monday
to Friday week — all four are properties of the calendar buffer, not settings.  =w= anchors the
new range on the middle of the old one, because a range shorter than a week /ends/ on the date
it is given and anchoring on the first day walked a week backwards per press.  =/= searches what
is on screen — every block drawn in the week, timed ones inside the canvas hours and all-day
ones inside the lanes the rail is showing — and only moves the cursor, so the view it found the
block in is the view it leaves behind.

Working hours are tinted and so are weekends, both by an =:after= advice on the background
renderer: the package's own weekend fill is a 2.5% blend that no theme of mine makes visible,
and it has no notion of working hours at all.  That advice is the piece most likely to break on
an update — it is positional in the renderer's arguments — so it draws inside
~with-demoted-errors~ and a bad day costs the tint, not the calendar.

Untimed repeats are dropped, because habits have no time of day and so landed on the all-day
rail every single day whether or not they were due — a repeating /meeting/ has a time and still
draws.  What is left of the rail gets two lanes, and =z= cycles that open far enough to read the
=+31 more=, then shut, then back to two.  Open stops at half the window: the rail is the header
line, a single SVG, so it cannot scroll and a tall one just eats the calendar.

Blocks lead with their [[*roam-refile-category][category icon]], the same glyph the agenda puts in front of the line, so
which project an hour belongs to is readable without reading it.  It is looked up with
~org-get-category~ and kept in the event's metadata rather than glued onto the title, and only a
non-ASCII first character counts — a file with no category falls back to its own name, and the
=t= of =tasks.org= is not an icon.

Titles are drawn through ~org-link-display-format~, since a heading that is mostly a link read as
=[[https://k2space.atl…= on the block.  Both draw functions are wrapped and each gets a copy of the
block, so the stripping is drawing only and =e= still renames against the real heading.  A title
that had a link is underlined — whole, not just the linked run — which is enough to tell the
entries carrying a ticket from the ones that don't, and =C-c C-o= opens it.

Colour is ~ads/org-timegrid-color~ on the package's own ~org-timegrid-org-color-function~ hook: a
=TIMEGRID_COLOR= property wins, then the tag map.  Property values name a colour in
~org-timegrid-colors~, so =purple= is the palette's purple rather than the X11 one.  Tags are read
through ~org-get-tags~ rather than the element's own ~:tags~, so they inherit from =#+FILETAGS= and
from parent headings the way the agenda does — the meeting file is yellow without every heading
...
** org-transclusion
[[https://github.com/nobiot/org-transclusion][org-transclusion]] embeds the content of one org file (or heading) inside another via ~#+transclude:~
links.  I use it so my daily can pull in work items that physically live in a seperate — the text
never lands in the personal repo, only the transclusion link does, and it renders inline in the
daily on the work machine.

Auto-rendered in dailies.  The =e= prefix is for *embed*.
...
*** transclusion quote only

My quote notes are usually files so I can reference them elsewhere if I want to.  Usually all I want
to embed is the quote itself not the title, properties, or attribution.  org-transclusion has no
built-in way to grab a bare, unnamed block reached by an =id:= link so i've had claude make
=:only-quote= flag that finds the first quote block in the source and transcludes only that:

#+begin_example
,#+transclude: [[id:8ff6cc01-bcfe-48f8-8346-11b7dc42c59e]] :only-quote
#+end_example

Works with =id:= and =file:= links; no =#+name:= required in the source.  It hooks
into org-transclusion's three extension points: one to parse the keyword, one to
re-serialise it on refresh, and one to build the payload.

Bound to =oeK=, this mirrors =org-transclusion-make-from-link= (=oek=) but tacks
=:only-quote= onto the keyword, so a link at point becomes a quote-only
transclusion in one step.
...
Only here for [[*copy (yank) as markdown][copy as markdown]].  Stock ~ox-md~ writes tables as raw HTML and code as an indented
block; the GitHub backend gets pipe tables and fences carrying the language, which is what the
places I paste into expect.
...
** pcre2el
[[https://github.com/joddie/pcre2el][pcre2el]]'s ~pcre-mode~ reads the regexps I type interactively as PCRE and hands Emacs
the translation — UI only, the regexps in this config stay Emacs syntax.  It advises
~read-regexp~ (~occur~, ~replace-regexp~, ~query-replace-regexp~, ~keep-lines~,
~dired-mark-files-regexp~ ...) and ~isearch~, where evil's ~/~ goes.  Not covered:
evil's ~:%s~ and ~consult-line~.  ~M-x pcre-mode~ for the odd Emacs-only construct
(~\_<~, ~\s-~).

~rxt-mode~ has the translation commands (~C-c / /~ explain, ~C-c / e p~ elisp to PCRE),
in ~prog-mode~ only since ~C-c /~ is ~org-sparse-tree~ in org.
...
*** Evil search arity fix

pcre2el's own ~evil-search-function~ advice is an old-style ~defadvice~ that hardcodes
the argument list ~(forward regexp-p wrap)~.  Evil has since added a fourth ~predicate~
argument, which ~evil-isearch-function~ passes, so every ~/~ died with
~wrong-number-of-arguments~.  Same behaviour, rewritten with ~advice-add~ and ~&rest~ so
the next signature change passes through.
...
~consult~ doesn't use ~read-regexp~, so ~consult-ripgrep~ needs its own compiler.
ripgrep understands the input as typed; only the Emacs side gets translated.
...
** pdf-tools
[[https://github.com/vedang/pdf-tools?tab=readme-ov-file#installing-pdf-tools][pdf-tools]] renders PDFs with poppler, which gets us real text selection, search and annotations.
The =epdfinfo= server is compiled on first install and needs =automake=, =autoconf=, =pkg-config= and
...
=org-file-apps= hands PDFs to the OS viewer by default, which means an org link or ~C-c C-a o~ on
an attachment bounces out to Preview.  Our entry is consed on the front, so it wins.
...
=pdf-view-themed-minor-mode= paints the page in the =default= face's colours, so a PDF looks
inverted under =modus-vivendi= and normal under =modus-operandi=.  It only re-reads the theme on
save or revert though, so open buffers need a nudge when ~SPC t t~ fires.  Turning it off with
=zt= sticks: the refresh skips buffers where it isn't on, so a manual override survives a theme
switch.
...
I host my org notes, including my config on a [[id:F8D27334-4635-4E36-8C19-E62FB01DC949][Quartz]] site. It was a whole vibe-coded project: [[id:98189AF8-632D-4E1F-95A5-31349A3EDD19][Org
Mode Publishing]], which finally makes me feel less isolated in my notes.

I use this to quickly grab URLs and other utils related to the site
...
These hang off =SPC m=, the conventional major-mode prefix.  They used to live on =SPC l=, which
[[*lsp-mode][lsp-mode]] also claims — =SPC l d= was both =lsp-find-definition= and =rust-dbg-wrap-or-unwrap=,
and which one you got depended on keymap precedence.  Only the two editing helpers survive; the
cargo commands that were here (=rust-run=, =rust-test=, =rust-check=, =rust-compile=,
=rust-run-clippy=) do nothing useful in a repo that builds through Bazel.

=rust-ts-mode-map= rather than =rust-mode-map= because [[*treesit-auto][treesit-auto]] remaps =rust-mode= to
=rust-ts-mode=, so a =.rs= buffer never has =rust-mode-map= active.  With
=rust-mode-treesitter-derive= set above, =rust-ts-mode-map= is =rust-mode-map='s parent anyway,
so these stay reachable either way.
#+begin_src emacs-lisp
(ads/leader-keys
...
*** Rustfmt
The rustfmt config, tangled to =~/.config/rustfmt/rustfmt.toml=:
#+begin_src conf-toml :tangle ~/.config/rustfmt/rustfmt.toml :mkdirp yes :comments no
edition = "2024"
style_edition = "2024"
...
Batteries-included rust mode built on rust-lang's rust-mode - disabled in favor of plain
[[*rust][rust-ts-mode]] plus [[*lsp-rust][lsp-rust]].

It was never actually loading: =:after (rust-mode lsp-mode)= against a =:commands=-deferred
lsp-mode means the =eval-after-load= chain only completes once something has already started an
LSP session, by which point the =.rs= buffer's major mode is settled.  What rustic adds over that
is cargo integration - =rustic-cargo-build=, =-test=, =-clippy= - which is exactly the part that
doesn't apply in a Bazel repo.  It also claims =\.rs= in =auto-mode-alist= for =rustic-mode=,
which would quietly bypass the =rust-ts-mode= hooks the rest of this config hangs off.
...
I use [[https://track.toggl.com][toggl track]] to track my time, through [[file:~/git/emacs-toggl/README.org][emacs-toggl]], which I wrote because the
CLI pipeline this used to run on could not tell me whether anything had actually
happened.  ~async-shell-command~ returns immediately, so the old
~toggl-start-timer~ echoed back the arguments I had typed and called that
success; meanwhile the sketchybar plugin polled the CLI on a 60 second cache, so
a timer took up to a minute to show up.  Between the two I stopped trusting it
and stopped tracking.

The package talks to the v9 API over HTTP and reports only what the response
body says.  Projects come from the org tree rather than a list I maintain by
hand: the roam node is the project, the clocked headline is the description.

**** saved timers

Most tracking comes from clocking in, but some things I start by hand and never
want to retype.  These used to be CLI argument strings passed to
~toggl-start-timer~; they are now ~(DESCRIPTION PROJECT [BILLABLE [TAGS]])~ handed
to the real command, so a project name that has drifted fails loudly instead of
quietly starting an unprojected entry.

~toggl-start-saved~ (~f~ in the transient) offers them annotated with the project
each files under and grouped by it, so picking one does not mean remembering
where it lands.  ~toggl-check-saved-timers~ reports any pointing at a project that
no longer exists.

This has to come *before* the ~use-package~ block, which reads it in ~:custom~.
Work entries get appended in [[file:~/git/konfig/readme.org][konfig]].
...
Pulled from [[https://github.com/andanao/emacs-toggl-track][its own repo]] rather than a path on disk, so a fresh machine gets it
without me remembering to clone anything.  Private repo, so the SSH remote.

Note this means Emacs loads the ~package-vc~ checkout under
~package-user-dir~, /not/ ~~/git/emacs-toggl~.  To hack on it and have the running
Emacs see the edits, point ELPA back at the working copy once:

#+begin_src emacs-lisp :tangle no
(package-vc-install-from-checkout "~/git/emacs-toggl" "toggl")
#+end_src
...
[[*sketchybar][sketchybar]] used to poll the CLI on a 60 second cache, which is why starting a
timer appeared to do nothing.  Pushing the state as soon as the server confirms
it makes the bar update in under a second.

~toggl-entry-changed-functions~ fires only on real transitions — start, stop,
switch — not on every refresh, so this does not redraw needlessly.
...
Python is in =treesit-auto-langs= because every Python hook in this config targets
=python-ts-mode= - including =(python-ts-mode . lsp-deferred)= in [[*lsp-mode][lsp-mode]].  Without the remap a
=.py= file opens in plain =python-mode=, none of those hooks fire, and Python LSP silently does
nothing.  Python uses treesit-auto's stock recipe; only Rust needs one spelled out here, to pin a
grammar revision.
...
~scroll-margin~ is 2 where ultra-scroll suggests 0, so point is kept off the last line — which
pixel scrolling often leaves half cut by the mode line — and has two lines of padding under it.
...
Recent vertico builds call ~set-local~, which Emacs 31 provides natively.  The
Emacs 30 compat shim was removed on the upgrade to 31.

~vertico-multiform-mode~ picks a display per completion category.  Only [[*jinx][jinx]] asks for one: its
suggestions are a pile of short words, and a grid shows most of them — and all the =1= to =9= keys
that select them — without scrolling.
...
Select-then-move window resizing. If only one separator exists on an axis, move
it directly. If multiple separators exist, first keypress selects, subsequent
keypresses move. SPC deselects.
...
#+end_src
** org-attach dir in windows explorer
#+begin_src emacs-lisp
...
First block of mac.el, so it carries the lexical-binding cookie.  ~:comments no~
keeps org's link comment from displacing it off line 1.
...
** Modifier Keys
Remap modifiers for a more ergonomic layout: Command becomes Control, Option becomes Meta, and Control becomes Meta.
#+begin_src emacs-lisp
...
** Frame Appearance
Remove the title bar icons for a cleaner look.
#+begin_src emacs-lisp
(add-to-list 'default-frame-alist '(undecorated . t))
#+end_src

** Homebrew PATH
GUI apps on macOS don't inherit the shell's PATH. Add the bin directories to both =exec-path= (for
Emacs finding executables) and =PATH= environment variable (for subprocesses like git hooks).

Homebrew isn't the only one that matters: =~/.cargo/bin= holds the rustup shims that
[[*lsp-rust][lsp-rust]] resolves =rust-analyzer= through, and =~/.local/bin= holds =uv='s tools including
[[*lsp-pyright][basedpyright-langserver]].  Without these, both language servers are simply not found and lsp-mode
fails to start with no obvious reason why.  Missing directories are dropped rather than added, so
this is safe on a machine that doesn't have all three.

nvm installs per-version, so the newest =~/.nvm/versions/node/*/bin= is globbed in rather than
named; without it [[*agent-shell][agent-shell]] can't find its ACP adapter.

=/Library/TeX/texbin= (MacTeX) is listed *last* deliberately -- each iteration prepends, so the
final entry ends up frontmost, and it has to beat Homebrew's =dvisvgm=, which can't find
=texmf.cnf= and dies on every TikZ picture.  MacTeX's own =dvisvgm= needs =LIBGS= to find
Ghostscript.  See [[*org latex preview][org latex preview]].
...
The osascript runs detached: synchronously it stalled every toggle for as long as System Events
took to answer, and a failure aborted the rest of the theme hook.
